...
public class RoomService {
    ...
    public String findAvailableRoomId(final BookingRequest bookingRequest) {
    ...
    public void bookRoom(final String roomId) {
        final Room room = roomAvailability.entrySet().stream()
    ...
    public void unbookRoom(final String roomId) {
        final Room room = roomAvailability.entrySet().stream()
                .filter(entry -> entry.getKey().getId().equals(roomId) && !entry.getValue())
                .findFirst()
                .map(entry -> entry.getKey())
